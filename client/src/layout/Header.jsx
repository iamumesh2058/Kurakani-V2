...
const Header = () => {
    const [showtLogout, setShowLogout] = useState(false);
                ...
                <nav className="header__menu">
                    {
                        currentUser ?
                            ...
                            <div className="header__user">
                                <Link href="{% url 'user-profile' request.user.id %}">
                                    <div className="avatar avatar--medium active">
                                        {
                                            currentUser.avatar ?
                                                <img src={`http://localhost:7000/${currentUser?.avatar}`} />
                                                :
                                                <img src={Avatar} />
                                        }
                                    </div>
                                    <p>{currentUser.name} <span>@{currentUser.username}</span></p>
                                </Link>
                                <button className="dropdown-button" onClick={() => setShowLogout(!showtLogout)}>
                                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
                    ...
                    <div className={`dropdown-menu ${showtLogout ? "show" : ""}`}>
                        ...
                        <Link className="dropdown-link" onClick={(logoutUser)}>
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
